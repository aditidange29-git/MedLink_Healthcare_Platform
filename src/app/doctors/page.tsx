...
interface Specialist {
  title: string; specialty: string; location: string;
  expertise: string; description: string; availability: string;
  phone?: string; doctorId?: number;
}
...
function FindDoctorsContent() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 py-12 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="skeuo-card mb-8">
          <div className="grid md:grid-cols-4 gap-4">
            ...
            <button onClick={() => setFilters({ specialty: "", city: "", search: "" })} className="skeuo-button">
              Clear Filters
            </button>
          </div>
          <div className="mt-4 text-sm text-slate-600">
            Showing {filtered.length} of {doctorsData.length} doctors
          </div>
        </div>
            ...
            >
              ...
              <div className="space-y-2 mb-4">
                <div className="flex items-center gap-2 text-sm text-slate-600">
                  <MapPin size={16} className="text-green-500" /><span>{doctor.location}</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-slate-600">
                  <Phone size={16} className="text-green-500" /><span>{doctor.phone}</span>
                </div>
                <div className="text-sm text-slate-500">Experience: {doctor.experience}</div>
              </div>
              <button className="w-full bg-gradient-to-r from-green-600 to-emerald-600 text-white py-3 rounded-xl font-bold hover:scale-105 transition-transform shadow-lg">
                Know More
              </button>
            </div>
          ))}
      ...
      {selectedDoctor && (
        <AppointmentBooking specialist={selectedDoctor} onClose={() => setSelectedDoctor(null)} />
      )}
    </div>
  );
}
